...
export default function DocsConverter({ onSuccess }: { onSuccess: () => void }) {
  ...
  const loadFiles = async () => {
    try {
      const res = await fetch("/api/v1/conversions/pdf-to-word/files");
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await fetch("/api/v1/conversions/pdf-to-word", {
        method: "POST",
        body: formData,
      });
        ...
        {loading && (
          <div className="mt-6 text-center p-8 bg-[rgba(30,30,35,0.8)] rounded-xl border border-white/5">
            ...
            <p className="text-gray-400">
              Processing your PDF file and extracting text...
            </p>
          </div>
        )}
                ...
                {files.map((f, idx) => (
                  ...
                  >
                    ...
                    <td className="p-5 border-b border-white/5">
                      <a
                        href={`/api/v1/conversions/pdf-to-word/files/${f.filename}`}
                        download
