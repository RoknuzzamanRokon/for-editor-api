    ...
    table: {
      ...
      rows: [
        [
          ...
          "The bearer token was not supplied, expired, or could not be validated.",
        ],
        [
          "403 Forbidden",
          "Role or permission block",
          "The caller is authenticated but cannot access the route or action.",
        ],
        [
          "404 Not Found",
          "Missing user, request, or deploy resource",
          "Target entities are validated and missing resources return explicit not-found errors.",
        ],
        [
          "422 Validation Error",
          "Bad payload or query shape",
          "Input schema, paging values, or upload constraints failed validation.",
        ],
        [
          "500 Internal Server Error",
          "Server-side processing or deploy issue",
          "Unexpected backend failures, conversion engine problems, or deploy script problems.",
        ],
      ],
    },
  },
    ...
    table: {
      ...
      rows: [
        [
          "v3 conversions",
          "Points-based cost",
          "Successful requests consume the configured per-request points rule for eligible users.",
        ],
        [
          "Retry safety",
          "Idempotency advised",
          "Send Idempotency-Key on conversion POST calls to avoid duplicate processing or duplicate charging.",
        ],
        [
          "Admin and deploy",
          "Role-gated access",
          "Administrative reads and deploy operations are protected before execution.",
        ],
      ],
    },
  },
          ...
          >
            <span
              ...
              style={{
                ...
                color: methodTextColor[item.method],
              }}
            >
              {item.method}
            </span>
      ...
      <table className="min-w-[720px] w-full text-left text-sm">
        <thead style={{ background: surfaceColor }}>
          <tr>
            {table.headers.map((header) => (
              <th
                key={header}
                className="px-5 py-4 text-xs font-bold uppercase tracking-[0.2em] sm:px-6"
        ...
        <tbody>
          {table.rows.map((row) => (
            ...
            >
              <td
                className="px-5 py-4 font-semibold sm:px-6"
...
export default function Page() {
  ...
  const [selectedTesterId, setSelectedTesterId] = useState(
    testerEndpoints[0].id,
  );
  const [authToken, setAuthToken] = useState("");
  const [requestBody, setRequestBody] = useState(
    testerEndpoints[0].defaultBody,
  );
  ...
  const selectedTesterEndpoint =
    testerEndpoints.find((endpoint) => endpoint.id === selectedTesterId) ??
    testerEndpoints[0];
  ...
  async function sendTestRequest() {
    ...
    try {
      const response = await fetch(
        `${baseUrl}${selectedTesterEndpoint.path}`,
        init,
      );
  ...
  function renderSection(section: SectionDefinition) {
    return (
      ...
      >
        <div
          className="rounded-[28px] border px-5 py-6 sm:px-7 sm:py-8 lg:px-9"
          style={endpointCard}
        >
          ...
          {section.codeSample ? (
            ...
            >
              <div
                ...
                style={{ borderColor: t.codeBlockBorder }}
              >
                <span
    ...
    >
      <div className="mx-auto max-w-10xl px-3 pb-6 sm:px-4 sm:pb-8 lg:px-6">
        <aside className="fixed bottom-6 left-6 top-32 hidden w-[260px] lg:block">
          <div
            className="h-full overflow-y-auto rounded-[28px] border px-5 py-6"
        ...
        <aside className="fixed bottom-6 right-6 top-32 hidden w-[300px] xl:block">
          <div
            className="flex h-full flex-col gap-4 overflow-hidden rounded-[13px] border px-5 py-5"
            style={{ background: t.bgSecondary, borderColor: t.border }}
          >
            ...
            <div className="flex-1 space-y-4 overflow-y-auto pr-1">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder={
                    selectedTesterEndpoint.requiresAuth
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder={
                    selectedTesterEndpoint.method === "GET"
                      ? "GET requests usually do not need a JSON body."
                      : '{\n  "key": "value"\n}'
                  }
              ...
              >
                <span className="material-symbols-outlined text-base">
                  {isSending ? "progress_activity" : "play_arrow"}
                </span>
              ...
              >
                ...
                <pre
                  className="mt-4 overflow-x-auto whitespace-pre-wrap break-words text-[11px] leading-6"
                  style={{ color: t.text }}
                >
        ...
        <main className="min-w-0 lg:ml-[284px] xl:mr-[324px]">
          ...
          <div className="space-y-6">
            <section
              className="rounded-[13px] border px-5 py-7 sm:px-7 sm:py-8 lg:px-10"
              style={{ background: t.bgSecondary, borderColor: t.border }}
            >
              <div className="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
                <div className="max-w-3xl">
                  ...
                  <p
                    ...
                    style={{ color: t.text }}
                  >
                    This page now tracks the current backend structure, with
                    grouped docs for v2 auth and users plus the active v3
                    platform routes for conversions, points, permissions,
                    dashboards, admin tooling, and deployment.
                  </p>
                </div>
              </div>
            </section>
